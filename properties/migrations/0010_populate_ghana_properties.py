...
def populate_ghana_properties(apps, schema_editor):
    """Populate database with 48 Ghana properties (3 per region)"""
    ...
    UnitType = apps.get_model('properties', 'UnitType')
    UnitTypePricing = apps.get_model('properties', 'UnitTypePricing')
    ...
    Review = apps.get_model('feedback', 'Review')
    ...
    properties_data = get_all_ghana_properties()
    
    for prop_data in properties_data:
        # Check if property already exists
        if Property.objects.filter(property_code=prop_data['property_code']).exists():
            continue
        ...
        if property_obj.property_type in ['HOSTEL', 'STUDENT_APARTMENT']:
            create_room_types(property_obj, prop_data, RoomType, RoomTypePricing, Room)
        else:
            create_unit_types(property_obj, prop_data, UnitType, UnitTypePricing)
        
        # Create proximity destinations
        ...
        if prop_data.get('is_premium', False):
            create_premium_reviews(property_obj, admin_user, User, Review)


def create_room_types(property_obj, prop_data, RoomType, RoomTypePricing, Room):
    for room_type_data in prop_data.get('room_types', []):
        ...
        create_physical_rooms(property_obj, room_type, room_type_data, Room)


def create_unit_types(property_obj, prop_data, UnitType, UnitTypePricing):
...
def create_physical_rooms(property_obj, room_type, room_type_data, Room):
    total_rooms = room_type_data['total_rooms']
...
def create_premium_reviews(property_obj, admin_user, User, Review):
    ...
    for first_name, last_name in ghanaian_names[:3]:
...
def get_all_ghana_properties():
    """Generate 48 properties (3 per each of Ghana's 16 regions)"""
    all_properties = []
    
    # Ashanti Region
    all_properties.extend(generate_ashanti_properties())
    # Brong-Ahafo Region
    all_properties.extend(generate_brong_ahafo_properties())
    # Central Region
    all_properties.extend(generate_central_properties())
    # Eastern Region
    all_properties.extend(generate_eastern_properties())
    # Greater Accra Region
    all_properties.extend(generate_greater_accra_properties())
    # Northern Region
    all_properties.extend(generate_northern_properties())
    # Upper East Region
    all_properties.extend(generate_upper_east_properties())
    # Upper West Region
    all_properties.extend(generate_upper_west_properties())
    # Volta Region
    all_properties.extend(generate_volta_properties())
    # Western Region
    all_properties.extend(generate_western_properties())
    # Ahafo Region
    all_properties.extend(generate_ahafo_properties())
    # Bono East Region
    all_properties.extend(generate_bono_east_properties())
    # North East Region
    all_properties.extend(generate_north_east_properties())
    # Oti Region
    all_properties.extend(generate_oti_properties())
    # Savannah Region
    all_properties.extend(generate_savannah_properties())
    # Western North Region
    all_properties.extend(generate_western_north_properties())
    
    return all_properties


def generate_ashanti_properties():
    base = {
...
def generate_brong_ahafo_properties():
    base = {
...
def generate_central_properties():
    base = {
...
def generate_eastern_properties():
    base = {
...
def generate_greater_accra_properties():
    base = {
...
def generate_northern_properties():
    base = {
...
def generate_upper_east_properties():
    base = {
...
def generate_upper_west_properties():
    base = {
...
def generate_western_properties():
    base = {
...
def generate_bono_east_properties():
    base = {
...
def generate_north_east_properties():
    base = {
...
def generate_savannah_properties():
    base = {
...
def generate_western_north_properties():
    base = {
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(populate_ghana_properties, migrations.RunPython.noop),
    ]
